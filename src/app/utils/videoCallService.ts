...
export class VideoCallService {
    ...
    async startVideoCall(roomId: string): Promise<MatrixCall> {
        ...
        try {
            ...
            throw new Error('Không thể truy cập micro hoặc camera: ' + (err instanceof Error ? err.message : 'Lỗi không xác định'));
        }
    ...
    async answerCall(call: MatrixCall): Promise<void> {
        console.log('Answering video call:', call.callId);
        this.cleanupCall();
        try {
            const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: true });
    ...
    onIncomingCall(callback: (call: MatrixCall) => void): () => void {
        ...
        const wrappedCallback = (call: MatrixCall) => {
            if (call.state === CallState.Ringing && call.type === 'video') {
                callback(call);
            } else {
                console.warn(`Ignoring incoming call ${call.callId} in unexpected state: ${call.state} or type: ${call.type}`);
            }
        };
