...
export class CallService {
  ...
  private setupCallListeners(call: MatrixCall): void {
    ...
    const onStateChange = (state: string) => {
      ...
      if (state === CallState.InviteSent && !this.timeoutId) {
        // Thiết lập timeout khi cuộc gọi đang ở trạng thái "invite_sent" (chờ người nhận bắt máy)
        this.timeoutId = setTimeout(() => {
          if (!this.isConnected && this.activeCall) {
            console.warn('Call timeout (no answer), forcing hangup for call:', call.callId);
            this.hangupCall();
          }
        }, 60000); // 60 giây
      } else if (state === CallState.Connected && !this.isConnected) {
        // Hủy timeout khi cuộc gọi đã kết nối thành công
        if (this.timeoutId) {
          clearTimeout(this.timeoutId);
          this.timeoutId = null;
          console.log('Timeout cleared as call is connected:', call.callId);
        }
        ...
        // Hủy timeout nếu cuộc gọi kết thúc trước khi timeout
        if (this.timeoutId) {
          clearTimeout(this.timeoutId);
          this.timeoutId = null;
        }
